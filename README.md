...
This repository is a **build specification and early implementation**, not a finished production engine. It contains a general MotionSpec schema, source extractors, validation and planning commands, a deterministic preview renderer, adapter contracts, synthetic examples, tests, and implementation milestones. A [desktop coding agent](docs/desktop-agent-workflow.md) such as Codex or Claude authors a whole-video plan and scene revisions; the local CLI compiles and renders them without an LLM API key. Stable scene IDs can also render as independent, hash-bound review clips for selective inspection after an edit. Still images and moving visual plates support bounded zoom, clockwise rotation, and x/y motion keyframes; directed scenes can include source-checked bar and line charts, structured animated counters, two-color backgrounds, directional whip entrances, approved narration, frame-placed sound effects, and a continuous music bed with fades and scene-level narration ducking. Typed scene edits can replace an approved shot or change text, color, layout, opacity, position, zoom, and plate rotation while preserving other scenes. A video importer converts supplied MP4/MOV/WebM/MKV files into exact-frame, hashed preview plates. Narrow After Effects, Illustrator, and Photoshop exporters produce editable subsets and passed native-app reopen checks. A limited Premiere XML interchange exporter lays out rendered scenes and beat markers and passed a native Premiere Pro 2026 import, save/reopen, and export test for a silent two-scene fixture. Full Adobe coverage remains a future milestone.

Factual briefs can produce a source-linked first-draft preview when each scene cites claims from a hashed [claim ledger](docs/research.md). The QA report requires semantic review of those claims before factual release; source matching alone does not establish truth.
