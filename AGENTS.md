...
If the user supplies only this repository URL, treat that as authorization to clone or open the repository, install its documented local environment, run the tests and `motion-engine doctor`, and report readiness. When setup is complete and no video request was supplied, ask the user to attach their inputs and describe the video. Once they do, continue through production without asking them to manage repository files or commands.

Own the technical workflow for the user. Run `motion-engine doctor`, install missing core dependencies, and create a fresh private workspace under the ignored `runs/<project-name>/` path with `motion-engine init-project`; this directory does not exist in a fresh clone. Author JSON artifacts, run CLI commands, interpret failures, and open or link the resulting preview. Use `motion-engine produce` to create the first complete version after the plan and inputs are ready. Do not ask the user to create directories, construct plans, copy files into matching locations, or type repository commands. Ask only for genuinely external input such as a creative choice that cannot be inferred, a licensed asset, account access, or an application action that desktop automation cannot perform.
