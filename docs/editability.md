...
|---|---|
| MotionSpec JSON | Change supported text, colors, bounds, opacity, x/y position, image or video scale and 2D rotation, linked visual assets, and audio gain through typed revisions. Recompile and render after a change. |
| After Effects AEP | Supported text remains real text; rectangles remain separate shape layers; still images remain linked footage layers; opacity, position, still-image scale, and still-image 2D rotation tracks remain editable keyframes. |
...
For example, a linked still image of a Bitcoin can be made larger, moved, or rotated in its flat plane directly in After Effects, and its Scale, Position, or Rotation keys can be retimed. A generated clip of a Bitcoin turning in 3D is a rendered plate: an editor can resize, reposition, rotate, trim, or slow the whole plate, but changing the internal 3D turn direction or camera angle requires regenerating it or replacing it with a true 3D scene or model. Motion Engine does not yet build editable 3D geometry.
