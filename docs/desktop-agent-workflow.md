...
2. Save the user's prompt as UTF-8 `project/prompt.txt`. Ingest relevant sources; preserve values and source locations. For a factual topic, verify the premise and important claims before composing narration. Write a [claim ledger](research.md) and run `motion-engine verify-claims`; inspect both source links and semantic support. The current first-draft compiler rejects `researchRequired: true`, so a fully verified factual video remains an unfinished product capability.
...
5. Run `motion-engine first-draft project/prompt.txt project/plan.json --project-id ID --output-spec project/first.motion.json --output-dir project/preview --assets project/assets.json`. Set canvas, frame rate, and preview scale for the actual brief. Run `motion-engine qa` and `motion-engine contact-sheet project/first.motion.json --render-dir project/preview --output-dir project/review`. Inspect every scene on the contact sheets and watch the MP4 for motion, timing, and audio. Correct the plan or assets and render a new version if visual quality is weak.
