...
3. Run `motion-engine director-schema --output project/director-schema.json`. Write `project/plan.json` with all scenes, frame durations, copy, visuals, and motion. Use a fresh project directory for each run. The plan is an agent-authored artifact; the CLI does not ask a model to create it. When preparing an After Effects target, optionally set plan-level `fontFamily` to a family verified in the installed app. The default is `DejaVu Sans`; the adapter fails if its requested Regular or Bold face cannot be resolved uniquely, and never silently changes fonts.
4. Fulfill `assetRequests` with rights-cleared user media, local procedural output, or an optional visual generation tool available to the agent. Inspect every asset and record its license and approval. Import moving clips with `motion-engine import-video`, then write a local [asset manifest](../examples/asset-catalog.manifest.json) with `id`, `kind`, relative `uri`, `license`, and `approved: true` for each reviewed file. Run `motion-engine catalog-assets project/asset-manifest.json --output project/assets.json --fps 30` to inspect media and fill hashes. Then run `motion-engine resolve-assets project/plan.json --assets project/assets.json --output project/ready-plan.json --fps 30` and use the ready plan for rendering. A 3D request needs explicit `generation.technique: "3d"` provenance on a moving plate and visible depth inspection. Keep exact text and data as deterministic layers.
