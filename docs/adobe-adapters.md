...
The installed Premiere Pro 2024 is version 24.5.0.57. Adobe's [Premiere UXP changelog](https://developer.adobe.com/premiere-pro/uxp/changelog/) places its first public beta in Premiere 25.2 and its official release in 25.6. Therefore the Premiere 24.x adapter must use a separately tested CEP/ExtendScript bridge or a documented interchange format; generating UXP code for that installed version would be an unsupported route. The 24.x execution bridge and the interchange fidelity for tracks, markers, and editable text are not yet proven. Keep both capability flags false until app-level tests pass. For newer installations, negotiate the documented UXP API version before use.
