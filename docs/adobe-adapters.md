...
Premiere Pro 2026 version 26.5.1 is now installed on the Windows development host. `adapters/premiere-uxp-probe` contains a command plugin for the first native test. It creates a scratch project from public media, saves, closes, reopens, and inspects a sequence, one video clip, a one-second marker, and the online media link. The plugin has **not** yet passed in the host. Its sequence is derived from a still image, so an explicit sequence profile and general MotionSpec timeline support remain unproved. The probe report is the evidence gate; source code or a passing mock alone does not establish compatibility.
