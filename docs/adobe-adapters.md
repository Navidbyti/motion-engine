...
Premiere Pro 2024 version 24.5.0.57 was used for the initial empty-project probe. Adobe's [Premiere UXP changelog](https://developer.adobe.com/premiere-pro/uxp/changelog/) places its first public beta in Premiere 25.2 and its official release in 25.6. Therefore a Premiere 24.x adapter would need a separately tested CEP/ExtendScript bridge or a documented interchange format; generating UXP code for that version would be an unsupported route. The 24.x execution bridge and interchange fidelity remain unproved. Premiere Pro 2026 is the current UXP development target.
...
Premiere Pro 2026 version 26.5.1 is now installed on the Windows development host. `adapters/premiere-uxp-probe` contains a command plugin for the first native test. It creates a scratch project from public media, sets 30 fps, saves, closes, reopens, and inspects the frame rate, one video clip, a one-second marker, and the online media link. The plugin has **not** yet passed in the host. Its sequence is derived from a still image, so explicit dimensions and general MotionSpec timeline support remain unproved. The probe report is the evidence gate; source code or a passing mock alone does not establish compatibility.
