...
`set_text` accepts a text element, replaces matching beat copy, and binds the new words to the revision request instead of retaining stale exact-text citations. `set_color` accepts text, rectangle, and bar/line chart elements. `set_bounds` replaces a visual element's `x`, `y`, `width`, and `height` with a finite rectangle inside the canvas; final safe-area and overflow QA still applies. `set_opacity` replaces the selected text, shape, image, or video's opacity keyframes with 1 to 20 absolute-frame values from 0 to 1. `set_position_x` and `set_position_y` replace frame-based position tracks for text, shapes, images, and video; values may begin up to one canvas width or height offscreen. `set_visual_zoom` accepts an image or video element and a list of absolute frame keyframes with finite scale values from 1 to 3; the visual remains clipped to its bounds. `set_visual_rotation` accepts image or video keyframes from -360 to +360 degrees, where positive values rotate clockwise in the preview. The earlier `set_image_zoom` operation remains supported for still images. `set_audio_gain` changes one audio element's `gainDb` to a finite value from -60 to +12 dB without changing its source or timing. These are deterministic edit operations for an agent to call after interpreting a prompt.
...
The desktop agent interprets the user's scene prompt and writes the typed JSON request. `motion-engine revise-and-render base.motion.json edit.json --output-spec next.motion.json --output-dir next-preview --review-dir next-review` applies the same checked revision, renders a new MP4, writes `next-preview/qa.json`, and creates verified contact sheets. No LLM API key is needed. The agent must inspect the output and report unsupported edit requests; factual-claim review remains an open gate.
