...
`motion-engine prompt-revise` adds an optional model planner for text, color, and visual zoom edits. It requires `OPENAI_API_KEY` and an explicit `--model`. The model sees only the selected scene's element IDs and current values; the application supplies the exact base spec hash and frame windows. It writes the user's original instruction into the typed request, then runs the same `revise-scene` validation and renders a new version. Unsupported operations fail with a reason. This route has simulated API tests; live model quality and factual-claim review remain open gates.
