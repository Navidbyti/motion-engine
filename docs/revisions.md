...
`set_text` accepts a text element, replaces matching beat copy, and binds the new words to the revision request instead of retaining stale exact-text citations. `set_color` accepts text, rectangle, and bar/line chart elements. `set_image_zoom` accepts an image element and a list of absolute frame keyframes with finite scale values from 1 to 3; the image remains clipped to its bounds. These are deterministic edit operations for an agent to call after interpreting a prompt. Natural-language editing, moving video plates, scene insertions, and research verification are separate open tasks.
