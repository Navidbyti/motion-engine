...
The CLI uses the same stage names: `motion-engine ingest`, `validate`, `plan`, `render`, `build`, `qa`, `package` when implemented. At present `ingest`, `validate`, `inspect`, `plan`, `freeze`, a deterministic visual `render`, an initial [QA gate](qa.md), and [preview bundling](preview-bundles.md) exist. `freeze` creates a verified [revision manifest](revisions.md); CLI renders bind to it. The renderer outputs silent MP4/PNG previews for its supported primitives. The preview bundle is a portable review artifact; native Adobe `build`, audio, full QA, and final approved packaging remain future work. Future multi-user deployments need authentication, access control, signed artifact URLs, audit logs, and server-side provider credentials.
