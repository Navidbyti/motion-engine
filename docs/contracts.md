...
The CLI uses the same stage names: `motion-engine ingest`, `validate`, `plan`, `render`, `build`, `qa`, `package` when implemented. At present `ingest`, `validate`, `inspect`, `plan`, and a deterministic visual `render` exist. The renderer outputs silent MP4/PNG previews for its supported primitives; native Adobe `build`, audio, full `qa`, and `package` remain future work. Future multi-user deployments need authentication, access control, signed artifact URLs, audit logs, and server-side provider credentials.
