...
See the [full product completion plan](product-roadmap.md) for every workstream and release gate. The extension contracts make the architecture general; actual format and visual coverage grows through tested implementations. The private fixture tests the harder end of the range but must never become a special-case code path.
