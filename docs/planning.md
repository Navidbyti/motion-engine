...
The built-in output adapters are marked `planned`; they do not build media or Adobe files yet. `plan` succeeds when it can describe the work and there is no unsupported required feature. The report still says `buildable: false`. Add `--require-buildable` to fail until every required target is backed by an available adapter. This makes the CLI useful for agent planning without claiming rendering exists.
