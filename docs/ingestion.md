...
| DOCX | body paragraphs and tables in document order; section headers and footers, including first/even page variants; embedded image relationships with part name, content type, byte count, and SHA-256 | Drawing placement and appearance, external image links, and tracked-change resolution require review; embedded image bytes are not exported by this command |
