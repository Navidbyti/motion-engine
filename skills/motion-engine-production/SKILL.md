...
- Select an explicit pacing profile. A full social reel uses `reel`, at least three purposeful scenes, and no scene longer than five seconds. Use `shot` only for a single visual beat and `explainer` for a longer narrative. Plan enough content to fulfill the requested duration instead of stretching sparse copy.
- Design an audio pass. Attach approved sound-effect WAV assets as frame-placed `soundEffects` cues with conservative gain, and inspect the final mix for clipping. Use `whip_left` or `whip_right` for a sharp directional entrance and two-color backgrounds when they serve the visual hierarchy.
